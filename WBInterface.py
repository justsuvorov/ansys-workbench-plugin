...
class WBInterface(object):
    ...
    def input_by_name(self, inp):
        ...
        self._log_(
            'Input successful: {} input(s) in {} Design Point(s)'.format(len(self._param_in), self.__DPs_imported), 1)
    ...
    def archive_project(self, filename=None, save_external_files=True, save_results=True, save_userfiles=True):
        ...
        args = dict(FilePath=wbpz_file, FailIfMissingFiles=False,
                    IncludeExternalImportedFiles=bool(save_external_files),
    ...
    def set_active_DP(self, dp):
        """Sets active DP"""
        try:
            workbench.Parameters.SetBaseDesignPoint(DesignPoint=dp)
        except:
            pass
    ...
    def status(self, suppress=False):
        ...
        if self.active:
            if self.failed_to_update:
                key = 2
            elif self.not_up_to_date:
                key = 1
            elif self.solved:
                key = 0
            else:
                key = 3
        elif self.failed_to_open:
            key = 4
        else:
            key = 5
        if not suppress: self._log_(msg_dict[key])
    ...
    # --------------------------------------------------------------------
    def save_overview(self, container, fpath, filename, width=0, height=0, fontfact=1, zoom_to_fit=True, view='iso',
                      module='Model', ignore_js_err=True):
        """
    ...
    # --------------------------------------------------------------------
    def save_mesh_view(self, container, fpath, filename, width=0, height=0, fontfact=1, zoom_to_fit=True, view='iso',
                       module='Model', ignore_js_err=True):
        """
        Saves mesh view
    ...
    def save_setups_view(self, container, fpath, fpref='Setup', width=0, height=0, fontfact=1, zoom_to_fit=True,
                         view='iso', module='Model', ignore_js_err=True):
        """
    ...
    # --------------------------------------------------------------------
    def save_figures(self, container, fpath, fpref='Result', width=0, height=0, fontfact=1, zoom_to_fit=False, view=0,
                     shade_mode='ShowUndeformedWireframe', module='Model', ignore_js_err=True):
        """
    ...
    # --------------------------------------------------------------------
    def save_animations(self, container, fpath, fpref='Animation', width=0, height=0, scale="auto", frames=20,
                        zoom_to_fit=True, view='iso', shade_mode='ShowWireframe', module='Model', ignore_js_err=False):
        """
        ...
        '''
        args = (
        self._winpath_js(fpath), height, width, scale_arg, self._bool_js(zoom_to_fit), frames, fpref, view, shade_mode)
    ...
    def _scale_eval(self, value):
        ...
        if isinstance(value, str):
            ...
            if value == 'auto':
                ...
                msg = 'five auto'
            elif value in undf_st:
                msg = 'undeformed'
            elif value == '0.5auto':
                msg = 'half auto'
            elif value in act_st:
                msg = 'true scale'
    ...
    def _shade_mode_eval(self, mode):
        ...
        if mode in ('shownowireframe', 1):
            res = 1
        elif mode in ('showundeformedwireframe', 2):
            res = 2
        elif mode in ('showundeformedmodel', 3):
            res = 3
        elif mode in ('showelements', 4):
            res = 4
        elif mode in ('showwireframe', 5):
            res = 5
        else:
            res = 0
...
class AsyncLogChecker(object):
    ...
    def stop(self):
        ...
        if not self.__is_watching:
            self._log_('Cannot execute stop command: watcher is inactive!')
        else:
                ...
                if not self.__current_file:
                    if self.__current_position:
                        ...
                        if self._div_symbol:
                            ...
                            if self.outfile:
                                with open(self.outfile, 'a') as g:
                                    g.write(msg_end(**args))
                            else:
                                self._log_(msg_end(**args), 2)
                        self._log_('Searching for a new watch file...')
